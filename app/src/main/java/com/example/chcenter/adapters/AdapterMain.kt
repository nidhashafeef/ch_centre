...
class AdapterMain(val context : Context, val data : ArrayList<ModelMainData>, val onItemClick : (Int, Int) -> Unit) : RecyclerView.Adapter<AdapterMain.MainItemVH>() {
    ...
    override fun onBindViewHolder(holder: MainItemVH, position: Int) {
        ...
        holder.TxtMainHeading.text = data[position].mainHeading
        holder.ReyclerNested.adapter = AdapterNested(context,data[position].dataList){innerPosition ->
    ...
    class MainItemVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val ReyclerNested: RecyclerView by lazy { itemView.findViewById<RecyclerView>(R.id.reyclerNested) }
        val TxtMainHeading: TextView by lazy { itemView.findViewById<TextView>(R.id.txtMainHeading) }
    }
}